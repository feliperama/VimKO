...
" -----------------------------------------------------------------------------
  ...
  function! RunTestsOnLeftPane(file_name_full_path)
    ...
    elseif( match(a:file_name_full_path, 'dm-customercare\/contexts.*.test.ts') != -1)
      let relative_path = fnamemodify(a:file_name_full_path, ":~:.")
      VimuxRunCommand("NODE_ENV=test:unit ./node_modules/.bin/mocha --require ts-node/register --full-trace --timeout 8000 --bail " . relative_path)
    elseif( match(a:file_name_full_path, 'ngr\/shared\/gateways\/__tests__.*.test.ts') != -1)
      let relative_path = fnamemodify(a:file_name_full_path, ":~:.")
      VimuxRunCommand("./node_modules/.bin/jest --verbose " . relative_path)
    elseif( match(a:file_name_full_path, 'ngr/handlers/refund-request/__tests__.*.test.ts') != -1)
      let relative_path = fnamemodify(a:file_name_full_path, ":~:.")
      VimuxRunCommand("./node_modules/.bin/jest --verbose " . relative_path)
    elseif( match(a:file_name_full_path, 'dm-customerorder\/order-management\/customer-returns.*.test.ts') != -1)
      let relative_path = fnamemodify(a:file_name_full_path, ":~:.")
      VimuxRunCommand("NODE_ENV=dev ./node_modules/.bin/jest --verbose " . relative_path)
